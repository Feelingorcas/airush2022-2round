...
class BERTClassifer(nn.Module) :
    ...
    def __init__(self, bert : BERT ,  args , hidden = 768 , freeze = True ):
        ...
        self.linear2 = nn.Sequential(nn.Linear(hidden , args.fc_dim) ,nn.Linear(args.fc_dim,10))
        # self.linear2 = nn.Sequential(nn.Linear(hidden, args.fc_dim) , nn.Dropout(p=0.5),  nn.ReLU() , nn.Linear(args.fc_dim, 10)) # best from now on
    ...
    def forward (self,x , segment_label  )  :
        ...
        seq_len = torch.sum(segment_label , dim = 1 ) -1
        # shop_len = segment_label.size(1)
        # x = self.linear1(x)
        # x = x[:,0,:] # or x = torch.mean(rand, dim =1 )
        # x B L ckdnj
        x = torch.mean(x[:,1:,:], dim=1)

        # x = x * segment_label[..., None]
        # # print(x)
        # x = torch.sum(x , dim =1 )
        # #
        # x = x / seq_len[..., None]
