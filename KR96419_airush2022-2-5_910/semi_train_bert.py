...
#######################################################################################################################
    ...
    teacher_model = BERT.BERTClassifer(bert= BERT.get_BERT(args), args=args, hidden=args.hidden_dim, freeze=args.freeze_bert).to(args.device)
...
######################################################################################################################
    ...
    ## 완벽하게 동일한 모델 하나 생성해냈음!
    args.fc_dim *= 2
    student_model = model = BERT.BERTClassifer(bert=teacher_model.bert, args=args, hidden=args.hidden_dim, freeze=args.freeze_bert).to(args.device)
