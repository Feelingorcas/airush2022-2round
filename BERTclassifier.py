...
class essemble_model(nn.Module ) :
    ...
    def __init__(self, args , module = None ):
        ...
        self.classifiers = nn.ModuleList(
            [nn.Sequential(nn.Linear(args.hidden_dim, args.fc_dim), GELU(), nn.Dropout(0.3),
                           nn.Linear(args.fc_dim, args.fc_dim), GELU(), nn.Linear(args.fc_dim, 10))
             for _ in range(args.k_fold)])
        if module is not None :
            self.bert.load_state_dict(module[0].bert.state_dict())
            for idx , classifier_ess in enumerate(self.classifiers) :
                classifier_ess.load_state_dict(module[idx].classifier.state_dict())

    def forward(self,x,segment_label):

        out  = None

        segment_label2 = (x > 0 and x != EOS).long()
